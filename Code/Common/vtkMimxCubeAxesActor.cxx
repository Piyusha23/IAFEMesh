/*=========================================================================
  ...
  Module:    $RCSfile: vtkMimxCubeAxesActor.cxx,v $
...
vtkStandardNewMacro(vtkMimxCubeAxesActor);
vtkCxxRevisionMacro(vtkMimxCubeAxesActor, "$Revision: 1.1 $");
vtkCxxSetObjectMacro(vtkMimxCubeAxesActor, Camera,vtkCamera);
...
vtkMimxCubeAxesActor::vtkMimxCubeAxesActor()
{
...
///////// added for custom axis numbering//////////////////////////////
  this->UseAxisRangeForXLabels = 0;
  this->UseAxisRangeForYLabels = 0;
  this->UseAxisRangeForZLabels = 0;

  this->UserXLabelRange[0] = this->Bounds[0];
  this->UserXLabelRange[1] = this->Bounds[1];
  this->UserYLabelRange[0] = this->Bounds[2];
  this->UserYLabelRange[1] = this->Bounds[3];
  this->UserZLabelRange[0] = this->Bounds[4];
  this->UserZLabelRange[1] = this->Bounds[5];
///////// added for custom axis numbering end ////////////////////////
...
void vtkMimxCubeAxesActor::ShallowCopy(vtkMimxCubeAxesActor *actor)
{
...
///////// added for custom axis numbering//////////////////////////////
  ...
  this->UseAxisRangeForXLabels = actor->UseAxisRangeForXLabels;
  this->UseAxisRangeForYLabels = actor->UseAxisRangeForYLabels;
  this->UseAxisRangeForZLabels = actor->UseAxisRangeForZLabels;
  this->UserXLabelRange[0] = actor->UserXLabelRange[0];
  this->UserXLabelRange[1] = actor->UserXLabelRange[1];
  this->UserYLabelRange[0] = actor->UserYLabelRange[0];
  this->UserYLabelRange[1] = actor->UserYLabelRange[1];
  this->UserZLabelRange[0] = actor->UserZLabelRange[0];
  this->UserZLabelRange[1] = actor->UserZLabelRange[1];
...
int vtkMimxCubeAxesActor::RenderOpaqueGeometry(vtkViewport *viewport)
{
...
void vtkMimxCubeAxesActor::AdjustAxes(double bounds[6], double xCoords[4][6], 
...
void vtkMimxCubeAxesActor::ReleaseGraphicsResources(vtkWindow *win)
{
  for (int i = 0; i < 4; i++)
    {
...
// *************************************************************************
void vtkMimxCubeAxesActor::GetBounds(double bounds[6])
{
  for (int i=0; i< 6; i++)
    {
...
// Compute the bounds
void vtkMimxCubeAxesActor::GetBounds(double& xmin, double& xmax, 
...
// Compute the bounds
double *vtkMimxCubeAxesActor::GetBounds()
{
  return this->Bounds;
}
...
void vtkMimxCubeAxesActor::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkMimxCubeAxesActor::TransformBounds(vtkViewport *viewport, 
...
bool vtkMimxCubeAxesActor::ComputeTickSize(double bounds[6])
{
...
// ****************************************************************************
//  Method: vtkMimxCubeAxesActor::AdjustValues
//
//  Purpose:
...
void vtkMimxCubeAxesActor::AdjustValues(const double bnds[6])
{
  char xTitle[64];
...
// ****************************************************************************
//  Method: vtkMimxCubeAxesActor::AdjustRange
//
//  Purpose:
...
void vtkMimxCubeAxesActor::AdjustRange(const double bnds[6])
{
...
int vtkMimxCubeAxesActor::Digits(double min, double max )
{
...
int vtkMimxCubeAxesActor::LabelExponent(double min, double max)
{
  if (min == max)
    {
    return 0;
...
void vtkMimxCubeAxesActor::BuildAxes(vtkViewport *viewport)
{
  double bounds[6]; 
  double pts[8][3];
  int i; 
...
void vtkMimxCubeAxesActor::SetNonDependentAttributes()
{
...
//
static int vtkMimxCubeAxesActorTriads[8][3] = {
  {mm,mm,mm}, {mm,Xm,Xm}, {Xm,mm,mX}, {Xm,Xm,XX},
  {mX,mX,mm}, {mX,XX,Xm}, {XX,mX,mX}, {XX,XX,XX}};
static int vtkMimxCubeAxesActorConn[8][3] = {{1,2,4}, {0,3,5}, {3,0,6}, {2,1,7},
...
// *************************************************************************
void vtkMimxCubeAxesActor::DetermineRenderAxes(vtkViewport *viewport)
{
  ...
  if (this->RenderCount++ == 0 || !(this->RenderCount % this->Inertia))
    ...
    if (this->FlyMode == VTK_FLY_CLOSEST_TRIAD)
      ...
      xloc = vtkMimxCubeAxesActorTriads[idx][0];
      yloc = vtkMimxCubeAxesActorTriads[idx][1];
      zloc = vtkMimxCubeAxesActorTriads[idx][2];
      ...
      for (xIdx=0, i=0; i<3; i++)
        {
        num = (pts[vtkMimxCubeAxesActorConn[idx][i]][1] - pts[idx][1]);
        den = (pts[vtkMimxCubeAxesActorConn[idx][i]][0] - pts[idx][0]);
        if (den != 0.0)
          {
          slope = num / den;
          }
        if (slope < minSlope && den > 0)
          {
          xIdx = vtkMimxCubeAxesActorConn[idx][i];
          yIdx = vtkMimxCubeAxesActorConn[idx][(i+1)%3];
          zIdx = vtkMimxCubeAxesActorConn[idx][(i+2)%3];
          xAxes = i;
          minSlope = slope;
          }
        }
      ...
      zAxes = (xAxes != 0 && yAxes != 0 ? 0 :
              (xAxes != 1 && yAxes != 1 ? 1 : 2));
      if (pts[vtkMimxCubeAxesActorConn[xIdx][zAxes]][2] <
          pts[vtkMimxCubeAxesActorConn[yIdx][zAxes]][2])
        {
        zIdx = xIdx;
        zIdx2 = vtkMimxCubeAxesActorConn[xIdx][zAxes];
        }
      else
        {
        zIdx = yIdx;
        zIdx2 = vtkMimxCubeAxesActorConn[yIdx][zAxes];
        }
      ...
      switch (xAxes)
        {
        case 0:
          xloc = vtkMimxCubeAxesActorTriads[mini][0];
          break;
        case 1:
          yloc = vtkMimxCubeAxesActorTriads[mini][1];
          break;
        case 2:
          zloc = vtkMimxCubeAxesActorTriads[mini][2];
          break;
        }
...
double vtkMimxCubeAxesActor::MaxOf(double a, double b)
{
  return (a > b ? a : b); 
}

double vtkMimxCubeAxesActor::MaxOf(double a, double b, double c, double d)
{
  return this->MaxOf(this->MaxOf(a, b), this->MaxOf(c, d));
}

inline double vtkMimxCubeAxesActor::FFix(double value)
{
  int ivalue = (int)value;
  return (double) ivalue;
}

inline double vtkMimxCubeAxesActor::FSign(double value, double sign)
{
  value = fabs(value);
  if (sign < 0.)
    {
...
// *******************************************************************
// Method: vtkMimxCubeAxesActor::AdjustTicksComputeRange
//
...
void vtkMimxCubeAxesActor::AdjustTicksComputeRange(vtkAxisActor *axes[4]) 
{
...
void vtkMimxCubeAxesActor::BuildLabels(vtkAxisActor *axes[4])
{
  char label[64];
  int i, labelCount = 0;
...
///////////////////////////////////////////
  double userRange[2] = { 0., 1. };
///////////////////////////////////////////

  vtkStringArray *labels = vtkStringArray::New();
////////////////////////////////////////////
  double userExtents = 0;
  double userSpecifiedDelta = 0;
  double userSpecifiedStartValue = 0;
////////////////////////////////////////////
...
/////////////////////////////////////////////////
      userExtents =
        (this->UseAxisRangeForXLabels)?this->UserXLabelRange[1]-this->UserXLabelRange[0]:extents;
      userSpecifiedStartValue =
        (this->UseAxisRangeForXLabels)?this->UserXLabelRange[0]:majorStart;
      userRange[0] = this->UserXLabelRange[0];
      userRange[1] = this->UserXLabelRange[1];
/////////////////////////////////////////////////
...
///////////////////////////////////////////////////
      userExtents =
        (this->UseAxisRangeForYLabels)?this->UserYLabelRange[1]-this->UserYLabelRange[0]:extents;
      userSpecifiedStartValue =
        (this->UseAxisRangeForYLabels)?this->UserYLabelRange[0]:majorStart;
      userRange[0] = this->UserYLabelRange[0];
      userRange[1] = this->UserYLabelRange[1];
////////////////////////////////////////////////////
      break;
...
/////////////////////////////////////////////////////
      userExtents =
        (this->UseAxisRangeForZLabels)?this->UserZLabelRange[1]-this->UserZLabelRange[0]:extents;
      userSpecifiedStartValue =
        (this->UseAxisRangeForZLabels)?this->UserZLabelRange[0]:majorStart;
      userRange[0] = this->UserZLabelRange[0];
      userRange[1] = this->UserZLabelRange[1];
/////////////////////////////////////////////////////
      break;
    }
...
//  val = majorStart; // removed for custom numbering 

///////////////////////////////////////////////////////start //////////////
// Recalculate start value and delta if user specified ranges for labels
  if(this->UseAxisRangeForXLabels||
     this->UseAxisRangeForYLabels||
     this->UseAxisRangeForZLabels)
    {
    double minorDeltaFromUserSpecifiedRange = 0;
    double minorStartValueFromUserSpecifiedRange = 0;
    ComputeLabelValuesForSpecifiedRange(userRange,
                                        userSpecifiedStartValue,
                                        userSpecifiedDelta,
                                        minorStartValueFromUserSpecifiedRange,
                                        minorDeltaFromUserSpecifiedRange);
    }
  else
    {
    userSpecifiedDelta = deltaMajor;
    }
  val = userSpecifiedStartValue;
////////////////////////////////////////////////////////// end //////
...
//    if (fabs(val) < 0.01 && extents > 1)
	  if (fabs(val) < 0.01 && userExtents > 1)
...
//    val += deltaMajor;
	  val += userSpecifiedDelta;//deltaMajor;
...
//
// ****************************************************************************
void vtkMimxCubeAxesActor::SetLabelScaling(bool autoscale, int upowX, int upowY, 
                                       int upowZ)
{
...
// ************************************************************************
void vtkMimxCubeAxesActor::SetXAxisLabelRange(double minValue, double maxValue)
{
  this->UserXLabelRange[0] = minValue;
  this->UserXLabelRange[1] = maxValue;
  this->UseAxisRangeForXLabels = 1;
  this->Modified();
}
// ************************************************************************
// Set the label range for vtkAxisActor for the X Axis
// ************************************************************************
void vtkMimxCubeAxesActor::SetYAxisLabelRange(double minValue, double maxValue)
{
  this->UserYLabelRange[0] = minValue;
  this->UserYLabelRange[1] = maxValue;
  this->UseAxisRangeForYLabels = 1;
  this->Modified();
}
// ************************************************************************
// Set the label range for vtkAxisActor for the X Axis
// ************************************************************************
void vtkMimxCubeAxesActor::SetZAxisLabelRange(double minValue, double maxValue)
{
  this->UserZLabelRange[0] = minValue;
  this->UserZLabelRange[1] = maxValue;
  this->UseAxisRangeForZLabels = 1;
  this->Modified();
}
// *******************************************************************
// Compute starting value and delta for user specified range
// *******************************************************************
void vtkMimxCubeAxesActor::ComputeLabelValuesForSpecifiedRange(double inRange[2],
                                                           ...
                                                           double& minorDelta)
{
 ...
 if (div != 1.)
  ...
  minorDelta = (fxt/div) / 10.;
  
  // Figure out the first major and minor tick locations, relative to the
  // start of the axis.
  if (sortedRange[0] <= 0.)
    {
      majorStartValue = majorDelta*(this->FFix(sortedRange[0]*(1./majorDelta)) + 0.);
      minorStartValue = minorDelta*(this->FFix(sortedRange[0]*(1./minorDelta)) + 0.);
    }
  else
    {
      majorStartValue = majorDelta*(this->FFix(sortedRange[0]*(1./majorDelta)) + 1.);
      minorStartValue = minorDelta*(this->FFix(sortedRange[0]*(1./minorDelta)) + 1.);
    }
}
