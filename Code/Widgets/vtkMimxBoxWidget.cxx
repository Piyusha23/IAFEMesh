/*=========================================================================
  ...
  Module:    $RCSfile: vtkMimxBoxWidget.cxx,v $
...
vtkCxxRevisionMacro(vtkMimxBoxWidget, "$Revision: 1.1 $");

vtkStandardNewMacro(vtkMimxBoxWidget);

vtkMimxBoxWidget::vtkMimxBoxWidget()
{
  this->State = vtkMimxBoxWidget::Start;
  this->EventCallbackCommand->SetCallback(vtkMimxBoxWidget::ProcessEvents);
  ...
  this->transaxes = vtkLocalAxesActor::New();
...
vtkMimxBoxWidget::~vtkMimxBoxWidget()
{
  ...
  this->transaxes->Delete();
}

void vtkMimxBoxWidget::SetEnabled(int enabling)
{
  if ( ! this->Interactor )
    {
  ...
  if ( enabling ) //------------------------------------------------------------
	...
	// added the actor for local axes in set enabled
	this->CurrentRenderer->AddActor(this->transaxes);
	this->CurrentRenderer->AddViewProp(this->transaxes);
	// added the actor for local axes in set enabled end 
	...
	this->CurrentRenderer->RemoveProp(this->transaxes);
    this->CurrentHandle = NULL;
...
void vtkMimxBoxWidget::ProcessEvents(vtkObject* vtkNotUsed(object), 
                                 unsigned long event,
                                 void* clientdata, 
                                 void* vtkNotUsed(calldata))
{
  vtkMimxBoxWidget* self = reinterpret_cast<vtkMimxBoxWidget *>( clientdata );
...
void vtkMimxBoxWidget::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkMimxBoxWidget::PositionHandles()
{
...
void vtkMimxBoxWidget::HandlesOn()
{
  for (int i=0; i<7; i++)
    {
...
void vtkMimxBoxWidget::HandlesOff()
{
  for (int i=0; i<7; i++)
    {
...
void vtkMimxBoxWidget::SizeHandles()
{
  double radius = this->vtk3DWidget::SizeHandles(0.0); // is the default size 1.15
  for(int i=0; i<7; i++)
    {
...
int vtkMimxBoxWidget::HighlightHandle(vtkProp *prop)
{
...
void vtkMimxBoxWidget::HighlightFace(int cellId)
{
  if ( cellId >= 0 )
    {
    vtkIdType npts;
...
void vtkMimxBoxWidget::HighlightOutline(int highlight)
{
  if ( highlight )
    {
...
void vtkMimxBoxWidget::OnLeftButtonDown()
{
  ...
  if (!this->CurrentRenderer || !this->CurrentRenderer->IsInViewport(X, Y))
    {
    this->State = vtkMimxBoxWidget::Outside;
    return;
    }
  ...
  if ( path != NULL )
    {
    this->State = vtkMimxBoxWidget::Moving;
    this->HighlightFace(
...
void vtkMimxBoxWidget::OnLeftButtonUp()
{
  if ( this->State == vtkMimxBoxWidget::Outside ||
       this->State == vtkMimxBoxWidget::Start )
    {
    return;
    }
...
void vtkMimxBoxWidget::OnMiddleButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
...
void vtkMimxBoxWidget::OnMiddleButtonUp()
{
  if ( this->State == vtkMimxBoxWidget::Outside ||
       this->State == vtkMimxBoxWidget::Start )
    {
...
void vtkMimxBoxWidget::OnRightButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if ( path != NULL )
    {
    this->State = vtkMimxBoxWidget::Scaling;
    this->HighlightOutline(1);
    this->HandlePicker->GetPickPosition(this->LastPickPosition);
    this->ValidPick = 1;
    }
...
void vtkMimxBoxWidget::OnRightButtonUp()
{
  if ( this->State == vtkMimxBoxWidget::Outside )
    {
    return;
...
void vtkMimxBoxWidget::OnMouseMove()
{
  ...
  // Process the motion
  if ( this->State == vtkMimxBoxWidget::Moving )
    {
    // Okay to process
    if ( this->CurrentHandle )
      {
  ...
  else if ( this->ScalingEnabled && this->State == vtkMimxBoxWidget::Scaling )
    {
...
void vtkMimxBoxWidget::MoveFace(double *p1, double *p2, double *dir, 
...
void vtkMimxBoxWidget::GetDirection(const double Nx[3],const double Ny[3], const double Nz[3], double dir[3])
{
  double dotNy, dotNz;
  double y[3];
...
void vtkMimxBoxWidget::MovePlusXFace(double *p1, double *p2)
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
  ...
  this->MoveFace(p1,p2,dir,x1,x2,x3,x4,h1);
	...
	//axespoints->InsertNextPoint(this->Points->GetPoint(1));
	//axespoints->InsertNextPoint(this->Points->GetPoint(3));
	//axespoints->InsertNextPoint(this->Points->GetPoint(4));
	
  //////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////

	//double axeslength[3];
	double axesdirection[3];
	...
	for(int i = 0; i< 3; i++)
	{
		double initialcoor[3];
		double finalcoor[3];
		this->Points->GetPoint(0,initialcoor);
		this->Points->GetPoint(point[i],finalcoor);

		for (int j = 0; j< 3; j++)
		{
			axesdirection[j] = finalcoor[j]-initialcoor[j];
		}			

		double length = sqrt(vtkMath::Distance2BetweenPoints(initialcoor,finalcoor));
		vtkMath::Normalize(axesdirection);
		for (int j = 0; j< 3; j++)
		{
			initialcoor[j]= initialcoor[j]+ (0.25*length*axesdirection[j]);
		}
		
		axespoints->InsertNextPoint(initialcoor);
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////////
	...
	this->transaxes->SetOrigin(this->Points->GetPoint(0));
	this->transaxes->SetAxesPoints(axespoints);
	this->transaxes->Modified();
	axespoints->Delete();
}

void vtkMimxBoxWidget::MoveMinusXFace(double *p1, double *p2)
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
...
void vtkMimxBoxWidget::MovePlusYFace(double *p1, double *p2)
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
...
void vtkMimxBoxWidget::MoveMinusYFace(double *p1, double *p2)
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
...
void vtkMimxBoxWidget::MovePlusZFace(double *p1, double *p2)
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
...
void vtkMimxBoxWidget::MoveMinusZFace(double *p1, double *p2)
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
...
void vtkMimxBoxWidget::Translate(double *p1, double *p2)
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
  double v[3];
...
void vtkMimxBoxWidget::Scale(double* vtkNotUsed(p1), double* vtkNotUsed(p2), 
...
void vtkMimxBoxWidget::ComputeNormals()
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
  double *p0 = pts;
  double *px = pts + 3*1;
...
void vtkMimxBoxWidget::GetPlanes(vtkPlanes *planes)
{
  if ( ! planes )
    {
    return;
...
void vtkMimxBoxWidget::Rotate(int X, int Y, double *p1, double *p2, double *vpn)
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
...
void vtkMimxBoxWidget::CreateDefaultProperties()
{
  // Handle properties
  ...
  this->SelectedFaceProperty->SetOpacity(0.0000000001);
  ...
  this->OutlineProperty->SetOpacity(0.0000000001);
  ...
  this->SelectedOutlineProperty->SetLineWidth(1.0);
  this->SelectedOutlineProperty->SetOpacity(0.0);
}

void vtkMimxBoxWidget::PlaceWidget(double bds[6])
{
  int i;
...
void vtkMimxBoxWidget::GetTransform(vtkTransform *t)
{
  double *pts = ((vtkDoubleArray *)this->Points->GetData())->GetPointer(0);
  double *p0 = pts;
  double *p1 = pts + 3*1;
...
void vtkMimxBoxWidget::SetTransform(vtkTransform* t)
{
  if (!t)
    {
...
void vtkMimxBoxWidget::GetPolyData(vtkPolyData *pd)
{
  pd->SetPoints(this->HexPolyData->GetPoints());
  pd->SetPolys(this->HexPolyData->GetPolys());
}

void vtkMimxBoxWidget::SetOutlineFaceWires(int newValue)
{
...
void vtkMimxBoxWidget::SetOutlineCursorWires(int newValue)
{
...
void vtkMimxBoxWidget::GenerateOutline()
{
...
void vtkMimxBoxWidget::SetInputActor (vtkActor *Actor){this->InputActor = Actor ;}


double* vtkMimxBoxWidget::GetCenter (){return this->Points->GetPoint(14) ; }