...
vtkCxxRevisionMacro(vtkKWMimxMainNotebook, "$Revision: 1.4 $");
...
void vtkKWMimxMainNotebook::SetLists()
{
  ...
  this->SurfaceMenuGroup->SetImageList(this->ImageMenuGroup->GetImageList());
  ...
  // lists for Boundary Condition Operations
  this->BoundaryConditionsMenuGroup->SetFEMeshList(this->FEMeshMenuGroup->GetFEMeshList());
  
  // Lists for Image Operations
  this->ImageMenuGroup->SetFeatureList(this->SurfaceMenuGroup->GetFeatureList());
}
//----------------------------------------------------------------------------
