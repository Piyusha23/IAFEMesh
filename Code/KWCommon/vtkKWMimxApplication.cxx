...
vtkCxxRevisionMacro(vtkKWMimxApplication, "$Revision: 1.3 $");
...
vtkKWMimxApplication::vtkKWMimxApplication()
{
  ...
  this->ResampleImageResolution = 1.0;
...
void vtkKWMimxApplication::AddAboutCopyrights(ostream &os)
{
  ...
  os << "and Bioengineering, National Institutes of Health." << std::endl << std::endl;

  os << "To reference IA-FEMesh please include the following citations:" << std::endl;
  os << "IA-FEMesh: an open-source, interactive, multiblock approach to anatomic finite element model development." << std::endl;
  os << "Grosland NM, Shivanna KH, Magnotta VA, Kallemeyn NA, DeVries NA, Tadepalli SC, Lisle C."  << std::endl;
  os << "Comput Methods Programs Biomed. 94(1):96-107, 2009."  << std::endl;
...
//----------------------------------------------------------------------------
double vtkKWMimxApplication::GetResampleImageResolution( )
{
  return this->ResampleImageResolution;
}

//----------------------------------------------------------------------------
void vtkKWMimxApplication::SetResampleImageResolution(double resolution)
{
  this->ResampleImageResolution = resolution;
  this->GetApplication()->SetRegistryValue(1, "MeshProperties", "ImageResolution", "%f", this->ResampleImageResolution);
  
  vtkKWMimxMainWindow *mimxWindow = vtkKWMimxMainWindow::SafeDownCast( this->GetNthWindow(0) );
  vtkKWMimxMainNotebook *mimxNotebook = mimxWindow->GetMainUserInterfacePanel()->GetMimxMainNotebook();
  vtkLinkedListWrapper *imageList = mimxNotebook->GetImageMenuGroup()->GetImageList();
  
  for (int i=0;i<imageList->GetNumberOfItems();i++)
    {
    vtkMimxImageActor *actor = vtkMimxImageActor::SafeDownCast(imageList->GetItem(i));
    actor->SetImageResolution(resolution);
    }
  
  mimxWindow->GetRenderWidget()->Render();
}
...
void vtkKWMimxApplication::LoadRegistryApplicationSettings()
{
  ...
  registryFlag = this->HasRegistryValue (1, "MeshProperties", "ImageResolution");
  if ( registryFlag )
  {
    this->ResampleImageResolution = static_cast<double> (
          this->GetFloatRegistryValue(1, "MeshProperties", "ImageResolution") );
  }
