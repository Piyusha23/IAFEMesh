...
vtkCxxRevisionMacro(vtkKWMimxApplicationSettingsInterface, "$Revision: 1.2 $");
...
vtkKWMimxApplicationSettingsInterface::vtkKWMimxApplicationSettingsInterface()
{
  ...
  this->ImageResolutionEntry = NULL;
}
...
vtkKWMimxApplicationSettingsInterface::~vtkKWMimxApplicationSettingsInterface()
{
  ...
  if (this->ImageResolutionEntry)
    {
    this->ImageResolutionEntry->Delete();
    this->ImageResolutionEntry = NULL;
    }
}
...
void vtkKWMimxApplicationSettingsInterface::Create()
{
  ...
  this->ImageResolutionEntry = vtkKWEntryWithLabel::New();
  this->ImageResolutionEntry->SetParent ( frame );
  this->ImageResolutionEntry->Create();
  this->ImageResolutionEntry->SetLabelText("Image Resolution:");
  this->ImageResolutionEntry->GetWidget()->SetRestrictValueToDouble( );
  this->ImageResolutionEntry->GetWidget()->SetCommand ( this, "ImageResolutionCallback");
  this->Script ( "pack %s -side top -anchor nw -padx 2 -pady 2 -fill x",
                this->ImageResolutionEntry->GetWidgetName());
...
void vtkKWMimxApplicationSettingsInterface::Update()
{
  ...
  if ( app != NULL )
  {
    ...
    double resolution = static_cast<double>(app->GetResampleImageResolution());
    if (this->ImageResolutionEntry)
    {
      this->ImageResolutionEntry->GetWidget()->SetValueAsDouble( resolution );
    }
...
//----------------------------------------------------------------------------
void vtkKWMimxApplicationSettingsInterface::ImageResolutionCallback(char *value)
{
  vtkKWMimxApplication *app
  = vtkKWMimxApplication::SafeDownCast(this->GetApplication());
  
  double resolution = atof( value );
  app->SetResampleImageResolution( resolution );
}
