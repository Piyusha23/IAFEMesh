...
vtkCxxRevisionMacro(vtkKWMimxViewProperties, "$Revision: 1.4 $");
...
void vtkKWMimxViewProperties::AddObjectList(vtkMimxActorBase *actor)
{
  ...
  switch (actor->GetDataType())
  {
    ...
    case ACTOR_POLYDATA_SURFACE:
      ...
      this->GetMimxMainWindow()->GetRenderWidget()->AddViewProp(actor->GetActor());
      break;
    case ACTOR_POLYDATA_LINE:
      this->MultiColumnList->GetWidget()->InsertCellText(rowIndex, 2, "Feature");
      this->GetMimxMainWindow()->GetRenderWidget()->AddViewProp(actor->GetActor());
      break;
...
void vtkKWMimxViewProperties::UpdateVisibility()
{
	for (int i=0; i<this->ObjectList->GetNumberOfItems(); i++)
	{
  int dataType = this->ObjectList->GetItem(i)->GetDataType();
  bool isVisible = false;
  
  if (dataType == ACTOR_FE_MESH)
    {
    vtkMimxMeshActor *meshActor = vtkMimxMeshActor::SafeDownCast(this->ObjectList->GetItem(i));
    isVisible = meshActor->GetMeshVisibility();
    }
  if (dataType == ACTOR_POLYDATA_SURFACE)
    {
    vtkMimxSurfacePolyDataActor *surfaceActor = vtkMimxSurfacePolyDataActor::SafeDownCast(this->ObjectList->GetItem(i));
    isVisible = surfaceActor->GetVisibility();
    }
  if (dataType == ACTOR_POLYDATA_LINE)
    {
    vtkMimxFeaturePolyDataActor *featureActor = vtkMimxFeaturePolyDataActor::SafeDownCast(this->ObjectList->GetItem(i));
    isVisible = featureActor->GetVisibility();
    }
  if (dataType == ACTOR_BUILDING_BLOCK)
    {
    vtkMimxUnstructuredGridActor *blockActor = vtkMimxUnstructuredGridActor::SafeDownCast(this->ObjectList->GetItem(i));
    isVisible = blockActor->GetVisibility();
    }
  if (dataType == ACTOR_IMAGE)
    {
    vtkMimxImageActor *imageActor = vtkMimxImageActor::SafeDownCast(this->ObjectList->GetItem(i));
    isVisible = imageActor->GetActorVisibility();
    }
