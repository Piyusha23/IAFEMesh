...
vtkCxxRevisionMacro(vtkKWMimxFEMeshMenuGroup, "$Revision: 1.5 $");
...
vtkKWMimxFEMeshMenuGroup::vtkKWMimxFEMeshMenuGroup()
{
  ...
  this->LoadMeshGroup = vtkKWMimxLoadMeshGroup::New();
}
...
vtkKWMimxFEMeshMenuGroup::~vtkKWMimxFEMeshMenuGroup()
{
  ...
  if(this->SaveMeshGroup)
	  this->SaveMeshGroup->Delete();
  ...
  if(this->LoadMeshGroup)
	  this->LoadMeshGroup->Delete();
}
//----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void vtkKWMimxFEMeshMenuGroup::LoadMeshCallback()
{
	//vtkKWMimxLoadMeshGroup *loadMeshGroup = vtkKWMimxLoadMeshGroup::New();
  this->LoadMeshGroup->SetParent( this->GetParent() );
	this->LoadMeshGroup->SetFEMeshList(this->FEMeshList);
	this->LoadMeshGroup->SetMenuGroup(this);
	this->LoadMeshGroup->SetMimxMainWindow(this->GetMimxMainWindow());
  this->LoadMeshGroup->Create();
}
...
//---------------------------------------------------------------------------------------
void vtkKWMimxFEMeshMenuGroup::SaveMeshCallback()
{
	if(!this->SaveMeshGroup)
	{
		this->SaveMeshGroup = vtkKWMimxSaveMeshGroup::New();
		this->SaveMeshGroup->SetParent( this->GetParent() );
		this->SaveMeshGroup->SetFEMeshList(this->FEMeshList);
		this->SaveMeshGroup->SetMenuGroup(this);
		this->SaveMeshGroup->SetMimxMainWindow(this->GetMimxMainWindow());
		this->SaveMeshGroup->Create();
	}
	else
	{
		this->SaveMeshGroup->UpdateObjectLists();
	}
	...
	this->GetApplication()->Script("pack %s -side top -anchor nw -expand n -padx 2 -pady 5 -fill x", 
								   this->SaveMeshGroup->GetBaseFrame()->GetWidgetName());
}
//------------------------------------------------------------------------------------------
...
void vtkKWMimxFEMeshMenuGroup::CreateFEMeshNodeSetCallback()
{
	if (!this->CreateFEMeshNodeSetGroup)
	{
		this->CreateFEMeshNodeSetGroup = vtkKWMimxCreateFEMeshNodeSetGroup::New();
